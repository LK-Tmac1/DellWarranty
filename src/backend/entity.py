...
class Warranty(object):
    ...
    def to_excel_data(self):
        return [self.service_ch, self.service_en, DateTimeUtil.parse_str_date(self.start_date),
                DateTimeUtil.parse_str_date(self.end_date), self.provider]
...
class DellAsset(object):
    ...
    def to_excel_data(self):
        return [self.svc_tag, self.machine_id, DateTimeUtil.parse_str_date(self.ship_date)]
    ...
    def serialize_txt_batch(dell_asset_list, output_dir):
        ...
        for da in dell_asset_list:
            ...
            output_path_list.append(file_name)
            FileUtil.save_object_to_path(da.serialize_txt(), os.path.join(output_dir, da.svc_tag))
